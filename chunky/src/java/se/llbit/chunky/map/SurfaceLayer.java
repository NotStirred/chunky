...
public class SurfaceLayer extends BitmapLayer {
   ...
   */
  public SurfaceLayer(int dim, ChunkData chunkData, BlockPalette palette, int[] heightmapData, int yMin, int yMax) {
    ...
    for (int x = 0; x < Chunk.X_MAX; ++x) {
      for (int z = 0; z < Chunk.Z_MAX; ++z) {
        ...
        int y = Math.min(chunkData.maxY() - 1, Math.min(yMax, heightmapData[z*Chunk.X_MAX + x]));
